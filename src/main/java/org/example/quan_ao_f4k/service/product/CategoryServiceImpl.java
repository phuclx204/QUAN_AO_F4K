...
public class CategoryServiceImpl implements CategoryService{
	private CategoryMapper brandMapper;
	...
	public ListResponse<CategoryResponse> findAll(int page, int size, String sort, String filter, String search, boolean all) {
		return defaultFindAll(page, size, sort, filter, search, all, SearchFields.CATEGORY, brandRepository, brandMapper);
	...
	public void exportExcel(HttpServletResponse response) throws Exception {
		List<Category> brands = brandRepository.findAll();
		...
		for (Category brand : brands) {
			...
			cell.setCellValue(brand.getDescription());
			cell.setCellStyle(dataStyle);
		}
	...
	public void exportPdf(HttpServletResponse response) throws Exception {
		...
		for (Category brand : brands) {
			...
			// Ô Mô tả
			cell = new PdfPCell(new Phrase(brand.getDescription(), fontCell));
			cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			table.addCell(cell);
		}
	...
	public List<CategoryResponse> findByStatusActive() {
		List<Category> categories = brandRepository.findByStatus(1);
		List<CategoryResponse> responses = new ArrayList<>();
		for (Category cate : categories) {
			CategoryResponse categoryResponse = new CategoryResponse(cate.getId(),cate.getName(),cate.getDescription(),cate.getStatus());
