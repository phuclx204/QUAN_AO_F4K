...
const  {formatNumberByDot, convert2Vnd} = getCommon()
...
$(document).ready(async function () {
    ...
    const productDetailId = document.querySelector('meta[name="product-detail-id"]').getAttribute("content");
    ...
    const loadOptionFilter = (res) => {
        addOptionFilter('/admin/color/active', $('#createColor'), [res.color.id]);
        addOptionFilter('/admin/size/active', $('#createSize'), [res.size.id]);
    }
    ...
    $actionSave.on("click", async function (e) {
        ...
        const data = {
            ...
            status: formCreateProductDefault.status
        }
    ...
    // get product detail
    const getData = async () => {
        const res = await $ajax.get("/admin/products/product-detail", {id: productDetailId})

        formCreateProductDefault.id = res.id;
        formCreateProductDefault.colorId = res.color.id;
        formCreateProductDefault.sizeId = res.size.id;
        formCreateProductDefault.quantity = res.quantity;
        formCreateProductDefault.price = res.price;
        formCreateProductDefault.status = res.status;
        ...
        $("#createPrice").val(formatNumberByDot(res.price + ''));
