...
$(document).ready(async function () {
    ...
    const $tableProduct = $('#products-table').DataTable({
        ...
        columns: [
            ...
            {data: 'name', title: 'Tên thương hiệu'},
            {
                data: 'status',
                title: 'Trạng thái',
    ...
    $(document).on("click", ".action-lock", async function (e) {
        ...
        if (isConfirmed.isConfirmed) {
            ...
            await $ajax.patch("/admin/brand/" + rowId, {status: STATUS_OFF});
    ...
    $(document).on("click", ".action-open", async function (e) {
        ...
        if (isConfirmed.isConfirmed) {
            const rowId = $(this).data("id");
            await $ajax.patch("/admin/brand/" + rowId, {status: STATUS_ON});
            $alter("success", "Thông báo", "Cập nhật thành công");
            reloadTable();
        }
    })
    ...
    $(document).on("click", ".action-update", function (e) {
        ...
        openModalAttributes("Cập nhật thuộc tính", "Tên thương hiệu", typeModal.type_brand, data);
    ...
    $(document).on("click", "#btnCreateProduct", function (e) {
        e.preventDefault();
        idRow.value = null;
        openModalAttributes('Thêm mới thuộc tính',"Tên thương hiệu", typeModal.type_brand);
        $modalAddAttributes.modal("show");
    })
